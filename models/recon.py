...
def transformer_reconstructer(inputs, memory, bias, mem_bias, params, state=None,
                        dtype=None, scope=None):
    #inputs: x
    #memory: decoder hidden
    with tf.variable_scope(scope, default_name="reconstructer", dtype=dtype,
                           values=[inputs, memory, bias, mem_bias]):
        x = inputs
        next_state = {}
        for layer in range(params.num_reconstructer_layers):
            layer_name = "layer_%d" % layer
            with tf.variable_scope(layer_name):
                layer_state = state[layer_name] if state is not None else None
...
def decoding_graph(features, state, mode, params):
    ...
    return_decoder_output = decoder_outputs
    # [batch, length, channel] => [batch * length, vocab_size]
    decoder_output = tf.reshape(decoder_outputs, [-1, hidden_size])
    logits = tf.matmul(decoder_output, weights, False, True)
    ...
    return loss, return_decoder_output

def reconstructing_graph(features, state, mode, params):
    if mode != "train":
        params.residual_dropout = 0.0
        params.attention_dropout = 0.0
        params.relu_dropout = 0.0
    ...
    if params.shared_source_target_embedding:
        with tf.variable_scope(tf.get_variable_scope(), reuse=True):
            recon_embedding = tf.get_variable("weights",
                                            [src_vocab_size, hidden_size],
                                            initializer=initializer)
    else:
        recon_embedding = tf.get_variable("recon_embedding",
                                        [src_vocab_size, hidden_size],
                                        initializer=initializer)

    if params.shared_embedding_and_softmax_weights:
        weights = recon_embedding
    else:
        weights = tf.get_variable("softmax", [src_vocab_size, hidden_size],
                                  initializer=initializer)

    # id => embedding
    # tgt_seq: [batch, max_tgt_length]
    sources = tf.gather(recon_embedding, src_seq) * (hidden_size ** 0.5)
    sources = sources * tf.expand_dims(src_mask, -1)

    # Preparing encoder and decoder input
    enc_attn_bias = layers.attention.attention_bias(tgt_mask, "masking")
    dec_attn_bias = layers.attention.attention_bias(tf.shape(sources)[1],
                                                    "causal") #TODO causal?
    # Shift left
    recon_input = tf.pad(sources, [[0, 0], [1, 0], [0, 0]])[:, :-1, :]
    recon_input = layers.attention.add_timing_signal(recon_input)

    if params.residual_dropout:
        keep_prob = 1.0 - params.residual_dropout
        recon_input = tf.nn.dropout(recon_input, keep_prob)

    decoder_output = state["decoder"]  

    recon_output = transformer_reconstructer(recon_input, decoder_output,
                                             dec_attn_bias, enc_attn_bias,
                                             params)


    # [batch, length, channel] => [batch * length, vocab_size]
    recon_output = tf.reshape(recon_output, [-1, hidden_size])
    logits = tf.matmul(recon_output, weights, False, True)
    ...
    if mode == "eval":
        return -tf.reduce_sum(ce * src_mask, axis=1) #TODO: 0?
...
def model_graph(features, mode, params):
    ...
    output, decoder_output = decoding_graph(features, state, mode, params)
    state = {
        "encoder": encoder_output,
        "decoder": decoder_output
    }
    recon_output = reconstructing_graph(features, state, mode, params)

    return output + recon_output


class Transformer_Reconstructer(interface.NMTModel):
    ...
    @staticmethod
    def get_name():
        return "transformer_reconstructer"
